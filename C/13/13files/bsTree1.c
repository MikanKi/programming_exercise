...
Node *newNode(char *word) {
    ...
    // 単語を節点のデータ領域へコピー
    new->word = strdup(word);   // mallocしてコピー
    if (new->word == NULL) {
...
Node *insertNode(Node *pt, char *word) {
    ...
    if(cmp<0){
        if(pt->left != NULL){
            insertNode(pt->left, word);
        } else {
            pt->left = newNode(word);
        }
    } else if(cmp>0){
        if(pt->right != NULL){
            insertNode(pt->right, word);
        } else {
            pt->right = newNode(word);
        }
    } else {
        pt->num++;
    }
...
int countNode(Node *pt) {
    if (pt == NULL)
        ...
        return countNode(pt->left) + countNode(pt->right) + 1;
    }
