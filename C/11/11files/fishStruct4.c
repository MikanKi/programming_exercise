...
int main(int argc, char *argv[]) {
    ...
    while (1) { // 無限ループにつき、グラフィックのウィンドウを閉じて終了
        ...
        for(j=0;j<NUM_FISH;j++){
            ...
            // 左右方向の壁との衝突判定
            if (fish[j].x < fish[j].size || fish[j].x > (XSIZE - fish[j].size)) {
                fish[j].dx = -fish[j].dx;
            }
        ...
        for(j=0; j<NUM_FISH-1;j++){
            for(k=j+1; k<NUM_FISH;k++){
                if(tooClose(fish[j],fish[k])==1){
                    fish[j].dx = fish[k].dx;
                    fish[j].dy = fish[k].dy;
                } else if(tooClose(fish[j],fish[k])==-1){
                    fish[k].dx = fish[j].dx;
                    fish[k].dy = fish[j].dy;
                }
            }
