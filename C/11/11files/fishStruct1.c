...
int main(int argc, char *argv[]) {
    ...
    while (1) { // 無限ループにつき、グラフィックのウィンドウを閉じて終了
        ...
        // 魚の描画
        if (fish.dx > 0.0) {
            fishRight(fish);   // 右向きの魚を描く
        } else {
            fishLeft(fish);    // 左向きの魚を描く
        }

        // 左右方向の壁との衝突判定
        if (fish.x < fish.size || fish.x > (XSIZE - fish.size)) {
            fish.dx = -fish.dx;
        }
        if (fish.y < fish.size || fish.y > (YSIZE - fish.size)) {
            fish.dy = -fish.dy;
        }
